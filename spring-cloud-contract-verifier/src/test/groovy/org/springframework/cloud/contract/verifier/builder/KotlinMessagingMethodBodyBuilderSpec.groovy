 ...
 * @author Tim Ysewyn
 */
class KotlinMessagingMethodBodyBuilderSpec extends Specification {
	...
	SingleTestGenerator.GeneratedClassData generatedClassData =
			new SingleTestGenerator.GeneratedClassData("foo", "com.example", new File(KotlinMessagingMethodBodyBuilderSpec.getResource(".").toURI()).toPath())
	...
	def setup() {
		properties = new ContractVerifierConfigProperties(
				assertJsonSize: true,
				generatedTestSourcesDir: new File(KotlinMessagingMethodBodyBuilderSpec.getResource(".").toURI()),
				generatedTestResourcesDir: new File(KotlinMessagingMethodBodyBuilderSpec.getResource(".").toURI())
		)
	}
...
\t@Inject
\tlateinit var contractVerifierObjectMapper: ContractVerifierObjectMapper
...
\t\t// then:
\t\t\tval response = contractVerifierMessaging.receive("activemq:output")
...
\t\t// and:
\t\t\tval parsedJson = JsonPath.parse(contractVerifierObjectMapper.writeValueAsString(response.getPayload()))
...
\tfun validate_foo() {
\t\t// given:
\t\t\tval inputMessage = contractVerifierMessaging.create(
\t\t\t\t\t"{\\"bookName\\":\\"foo\\"}"
\t\t\t\t\t\t, headers()
\t\t\t\t\t\t\t.header("sample", "header")
\t\t\t)
...
\t\t// then:
\t\t\tval response = contractVerifierMessaging.receive("jms:output")
\t\t\tassertThat(response).isNotNull()
...
'''
	...
	def "should generate tests without headers for [#methodBuilderName] with consumer / producer notation"() {
		given:
...
'''
	...
	def "should generate tests with message headers containing regular expression with backslashes for JUnit"() {
		...
		when:
			String test = singleTestGenerator(contractDsl)
		then:
			test.contains('assertThatJson(parsedJson).field("[\'eventId\']").matches("\\\\d+")')
	}
	...
	def "should generate tests with message headers containing regular expression with escapes for JUnit"() {
		...
		when:
			String test = singleTestGenerator(contractDsl)
		then:
			test.contains('val response = contractVerifierMessaging.receive("topic.rateablequote")')
			...
			test.contains('val parsedJson = JsonPath.parse(contractVerifierObjectMapper.writeValueAsString(response.getPayload()))')
...
\t\t// then:
\t\t\tval response = contractVerifierMessaging.receive("topic.rateablequote")
\t\t\tassertThat(response).isNotNull()
...
\t\t// then:
\t\t\tval response = contractVerifierMessaging.receive(toString())
\t\t\tassertThat(response).isNotNull()
...
\t\t// then:
\t\t\tval response = contractVerifierMessaging.receive("messageExchange")
\t\t\tassertThat(response).isNotNull()
...
\t\t// then:
\t\t\tval response = contractVerifierMessaging.receive("personEventsTopic")
\t\t\tassertThat(response).isNotNull()
